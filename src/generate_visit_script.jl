...
function generate_visit_script(structurename::String, adsorbate::String)
    ...
    @printf(f, "OpenDatabase(\"%s/PEGrid_output/%s.xyz\", 0)\n", homedir(), structurename)
    @printf(f, "AddPlot(\"Molecule\", \"element\", 1, 0)\n")
    @printf(f, "AddOperator(\"CreateBonds\", 0)\n")

    # vtz viz
    write(f, "# Open .vtk file\n")
    @printf(f, "OpenDatabase(\"%s/PEGrid_output/%s.vtk\", 0)\n", homedir(), structurename)
    @printf(f, "AddPlot(\"Mesh\", \"mesh\", 1, 0)\n")
    @printf(f, "DrawPlots()\n")
...
function do_vis(structurename::String, adsorbate::String)
    """
    Write Visit script for viz
    Write .vtk
    Write .xyz
    (not grid file though!)
    """
    generate_visit_script(structurename, adsorbate)
    write_unitcell_boundary_vtk(structurename)
    replicate_cssr_to_xyz(structurename)
end
