...
function ApplyBoundaryConditions(distance,framework)
	x = zeros(Float64,3) 
...
function CheckSurfaceAreaOverlap(probe, probe_size, f_xf, f_yf, f_zf,sigmas,framework)
	...
	for i = 1:array_size
		...
		if fAtom != fAtom_tmp
			...
			dr = ApplyBoundaryConditions(distance,framework)
...
function surfaceArea(probe::String, structurename::String, forcefieldname::String; num_sampling=1000,probe_size=3.31)
	"""
    ...
    forcefield = constructforcefield(forcefieldname, "N2", cutoff=12.8, mixingrules="Surface")
	...
	for i = 1:array_size
		...
		for j in 1:num_sampling
			...
			overlap = CheckSurfaceAreaOverlap(probe, probe_size, framework.xf[i], framework.yf[i], framework.zf[i],sigmas,framework)
			if overlap == false
				counted+=1
			end
		end
